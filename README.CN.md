...
- 🧠 **Seedance 浏览器代理补签**: Seedance 2.0 / 2.0 Fast 的提交请求会自动通过浏览器代理生成 `a_bogus`，规避 `ret=1019` / `shark not pass`
...
- Node.js 18+
- npm 或 yarn
- Chromium（本地运行 Seedance 2.0 / 2.0 Fast 前请执行 `npm run install:chromium`）
- Docker (可选)
...
# 安装 Chromium（Seedance 2.0 / 2.0 Fast 必需）
...
- ✅ **统一端口**：容器内外均使用5100端口
- ✅ **日志管理**：结构化日志输出
- ✅ **内置 Chromium**：镜像内已包含 Playwright 管理的 Chromium 与系统依赖
...
> **Seedance 补签说明**:
> - `jimeng-video-seedance-2.0` 和 `jimeng-video-seedance-2.0-fast` 的 `/mweb/v1/aigc_draft/generate` 提交会通过内置 BrowserService 发起。
> - BrowserService 会懒启动 headless Chromium，并在真实页面环境中让 bdms SDK 自动注入 `a_bogus`。
> - 仅最终提交请求走浏览器代理；图片生成、普通视频、上传、轮询、积分查询仍走 Node.js 直连。
> - 每个 token 独立复用浏览器会话，空闲 10 分钟后自动清理。
> - 首次 Seedance 请求会有一次性浏览器启动耗时。
